...
------------------------------------------------------------------------------- */
if (!function_exists('kryptex_trx_addons_theme_specific_setup1')) {
	add_filter( 'trx_addons_filter_components_editor', 'kryptex_trx_addons_theme_specific_components');
	function kryptex_trx_addons_theme_specific_components($enable=false) {
		return KRYPTEX_THEME_FREE
					? false		// Free version
...
if (!function_exists('kryptex_trx_addons_theme_specific_setup1')) {
	add_action( 'after_setup_theme', 'kryptex_trx_addons_theme_specific_setup1', 1 );
	add_action( 'trx_addons_action_save_options', 'kryptex_trx_addons_theme_specific_setup1', 1 );
	function kryptex_trx_addons_theme_specific_setup1() {
		if (kryptex_exists_trx_addons()) {
			add_filter( 'trx_addons_cv_enable',					'kryptex_trx_addons_cv_enable');
			add_filter( 'trx_addons_demo_enable',				'kryptex_trx_addons_demo_enable');
			add_filter( 'trx_addons_filter_edd_themes_market',	'kryptex_trx_addons_edd_themes_market_enable');
			add_filter( 'trx_addons_cpt_list',					'kryptex_trx_addons_cpt_list');
			add_filter( 'trx_addons_sc_list',					'kryptex_trx_addons_sc_list');
			add_filter( 'trx_addons_widgets_list',				'kryptex_trx_addons_widgets_list');
		}
	}
}

// CV
if ( !function_exists( 'kryptex_trx_addons_cv_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_cv_enable', 'kryptex_trx_addons_cv_enable');
	function kryptex_trx_addons_cv_enable($enable=false) {
...
// Demo mode
if ( !function_exists( 'kryptex_trx_addons_demo_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_demo_enable', 'kryptex_trx_addons_demo_enable');
	function kryptex_trx_addons_demo_enable($enable=false) {
...
// EDD Themes market
if ( !function_exists( 'kryptex_trx_addons_edd_themes_market_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_edd_themes_market', 'kryptex_trx_addons_edd_themes_market_enable')) {
	function kryptex_trx_addons_edd_themes_market_enable($enable=false) {
...
// API
if ( !function_exists( 'kryptex_trx_addons_api_list' ) ) {
	//Handler of the add_filter('trx_addons_api_list',	'kryptex_trx_addons_api_list');
	function kryptex_trx_addons_api_list($list=array()) {
...
// CPT
if ( !function_exists( 'kryptex_trx_addons_cpt_list' ) ) {
	//Handler of the add_filter('trx_addons_cpt_list',	'kryptex_trx_addons_cpt_list');
	function kryptex_trx_addons_cpt_list($list=array()) {
		// To do: Enable/Disable CPT via add/remove it in the list

		// If it's a free version - leave only basic set
		if (KRYPTEX_THEME_FREE) {
			$free_cpt = array('layouts', 'portfolio', 'post', 'services', 'currency', 'team', 'testimonials');
			foreach ($list as $k=>$v) {
...
// Shortcodes
if ( !function_exists( 'kryptex_trx_addons_sc_list' ) ) {
	//Handler of the add_filter('trx_addons_sc_list',	'kryptex_trx_addons_sc_list');
	function kryptex_trx_addons_sc_list($list=array()) {
...
// Widgets
if ( !function_exists( 'kryptex_trx_addons_widgets_list' ) ) {
	//Handler of the add_filter('trx_addons_widgets_list',	'kryptex_trx_addons_widgets_list');
	function kryptex_trx_addons_widgets_list($list=array()) {
...
if ( !function_exists( 'kryptex_trx_addons_menu_cache' ) ) {
	add_filter( 'trx_addons_filter_menu_cache', 'kryptex_trx_addons_menu_cache');
	function kryptex_trx_addons_menu_cache($list=array()) {
...
if (!function_exists('kryptex_trx_addons_localize_script')) {
	...
	function kryptex_trx_addons_localize_script($arr) {
		$arr['alter_link_color'] = kryptex_get_scheme_color('alter_link');
		return $arr;
	}
}
...
if ( !function_exists( 'kryptex_trx_addons_sc_type' ) ) {
	add_filter( 'trx_addons_sc_type', 'kryptex_trx_addons_sc_type', 10, 2);
	function kryptex_trx_addons_sc_type($list, $sc) {
...
if ( !function_exists( 'kryptex_trx_addons_sc_atts' ) ) {
	add_filter( 'trx_addons_sc_atts', 'kryptex_trx_addons_sc_atts', 10, 2);
	function kryptex_trx_addons_sc_atts($atts, $sc) {
		...
		if (in_array($sc, array('trx_sc_action', 'trx_sc_blogger', 'trx_sc_cars', 'trx_sc_courses', 'trx_sc_content', 'trx_sc_dishes',
								...
								'trx_sc_properties', 'trx_sc_services', 'trx_sc_currency', 'trx_sc_team', 'trx_sc_testimonials', 'trx_sc_title',
...
if ( !function_exists( 'kryptex_trx_addons_sc_map' ) ) {
	add_filter( 'trx_addons_sc_map', 'kryptex_trx_addons_sc_map', 10, 2);
	function kryptex_trx_addons_sc_map($params, $sc) {

		if (in_array($sc, array('trx_sc_services', 'trx_sc_currency'))) {
			$aa = $params['params'];
			foreach ($aa as $k => $v) {
		...
		if (in_array($sc, array('trx_sc_action', 'trx_sc_blogger', 'trx_sc_cars', 'trx_sc_courses', 'trx_sc_content', 'trx_sc_dishes',
			...
			$params['params'][] = array(
					'param_name' => 'scheme',
					'heading' => esc_html__('Color scheme', 'kryptex'),
					'description' => wp_kses_data( __('Select color scheme to decorate this block', 'kryptex') ),
					'group' => esc_html__('Colors', 'kryptex'),
					'admin_label' => true,
					'value' => array_flip(kryptex_get_list_schemes(true)),
					'type' => 'dropdown'
				);
		}
		// Param 'color_style'
		$param = array(
			...
			'heading' => esc_html__('Color style', 'kryptex'),
			'description' => wp_kses_data( __('Select color style to decorate this block', 'kryptex') ),
			'edit_field_class' => 'vc_col-sm-4',
			'admin_label' => true,
			'value' => array_flip(kryptex_get_list_sc_color_styles()),
			'type' => 'dropdown'
		);
...
if ( !function_exists( 'kryptex_trx_addons_sow_map' ) ) {
	add_filter( 'trx_addons_sow_map', 'kryptex_trx_addons_sow_map', 10, 2);
	function kryptex_trx_addons_sow_map($params, $sc) {

		// Param 'color_style'
		$param = array(
			'color_style' => array(
				'label' => esc_html__('Color style', 'kryptex'),
				'description' => wp_kses_data( __('Select color style to decorate this block', 'kryptex') ),
				'options' => kryptex_get_list_sc_color_styles(),
				'default' => 'default',
				'type' => 'select'
			)
		);
		if (in_array($sc, array('trx_sc_button')))
			kryptex_array_insert_after($params, 'size', $param);
		else if (in_array($sc, array('trx_sc_action', 'trx_sc_blogger', 'trx_sc_cars', 'trx_sc_courses', 'trx_sc_content', 'trx_sc_dishes',
								'trx_sc_events', 'trx_sc_form',	'trx_sc_googlemap', 'trx_sc_portfolio', 'trx_sc_price', 'trx_sc_promo',
								'trx_sc_properties', 'trx_sc_services', 'trx_sc_currency', 'trx_sc_team', 'trx_sc_testimonials', 'trx_sc_title',
								'trx_widget_audio', 'trx_widget_twitter')))
			kryptex_array_insert_after($params, 'title_align', $param);
		return $params;
	}
}

// Add classes to the shortcode's output
if ( !function_exists( 'kryptex_trx_addons_sc_output' ) ) {
	add_filter( 'trx_addons_sc_output', 'kryptex_trx_addons_sc_output', 10, 4);
	function kryptex_trx_addons_sc_output($output, $sc, $atts, $content) {
		
		if (in_array($sc, array('trx_sc_action'))) {
			if (!empty($atts['scheme']) && !kryptex_is_inherit($atts['scheme']))
				$output = str_replace('class="sc_action ', 'class="sc_action scheme_'.esc_attr($atts['scheme']).' ', $output);
			if (!empty($atts['color_style']) && !kryptex_is_inherit($atts['color_style']))
...
if ( !function_exists( 'kryptex_trx_addons_sc_item_title_tag' ) ) {
	add_filter( 'trx_addons_filter_sc_item_title_tag', 'kryptex_trx_addons_sc_item_title_tag');
	function kryptex_trx_addons_sc_item_title_tag($tag='') {
		return $tag=='h1' ? 'h2' : $tag;
	}
}

// Return args for the item's button
if ( !function_exists( 'kryptex_trx_addons_sc_item_button_args' ) ) {
	add_filter( 'trx_addons_filter_sc_item_button_args', 'kryptex_trx_addons_sc_item_button_args', 10, 3);
	function kryptex_trx_addons_sc_item_button_args($args, $sc, $sc_args) {
...
if ( !function_exists( 'kryptex_trx_addons_slider_title' ) ) {
	add_filter( 'trx_addons_filter_slider_title',	'kryptex_trx_addons_slider_title', 10, 2 );
	function kryptex_trx_addons_slider_title($title, $data) {
		$title = '';
		if (!empty($data['title'])) 
...
if ( !function_exists( 'kryptex_trx_addons_sc_googlemap_styles' ) ) {
	add_filter( 'trx_addons_filter_sc_googlemap_styles',	'kryptex_trx_addons_sc_googlemap_styles');
	function kryptex_trx_addons_sc_googlemap_styles($list) {
		$list['dark'] = esc_html__('Dark', 'kryptex');
		$list['extra'] = esc_html__('Extra', 'kryptex');
		return $list;
	}
}
...
if ( !function_exists( 'kryptex_trx_addons_tiny_mce_style_formats' ) ) {
	add_filter( 'trx_addons_filter_tiny_mce_style_formats', 'kryptex_trx_addons_tiny_mce_style_formats');
	function kryptex_trx_addons_tiny_mce_style_formats($style_formats) {
		...
		if (false && is_array($style_formats) && count($style_formats)>0 ) {
			foreach ($style_formats as $k=>$v) {
				if ( $v['title'] == esc_html__('List styles', 'kryptex') ) {
					$style_formats[$k]['items'][] = array(
								'title' => esc_html__('Arrow', 'kryptex'),
								'selector' => 'ul',
...
if ( !function_exists( 'kryptex_trx_addons_allow_override_header_image' ) ) {
	add_filter( 'kryptex_filter_allow_override_header_image', 'kryptex_trx_addons_allow_override_header_image' );
	function kryptex_trx_addons_allow_override_header_image($allow) {
		return kryptex_is_team_page() ? false : $allow;
	}
}

// Get thumb size for the team items
if ( !function_exists( 'kryptex_trx_addons_thumb_size' ) ) {
	add_filter( 'trx_addons_filter_thumb_size',	'kryptex_trx_addons_thumb_size', 10, 2);
	function kryptex_trx_addons_thumb_size($thumb_size='', $type='') {
		//if ($type == 'team-default')
			//$thumb_size = kryptex_get_thumb_size('med');
		return $thumb_size;
	}
}
...
if (!function_exists('kryptex_trx_addons_meta_box_fields')) {
	add_filter( 'trx_addons_filter_meta_box_fields', 'kryptex_trx_addons_meta_box_fields', 10, 2);
	function kryptex_trx_addons_meta_box_fields($mb, $post_type) {
		if (defined('TRX_ADDONS_CPT_TEAM_PT') && $post_type==TRX_ADDONS_CPT_TEAM_PT) {
			$mb['email'] = array(
				"title" => esc_html__("E-mail",  'kryptex'),
				"desc" => wp_kses_data( __("Team member's email", 'kryptex') ),
				"std" => "",
				"details" => true,
				"type" => "text"
			);
...
if ( !function_exists( 'kryptex_filter_get_list_input_hover' ) ) {
	add_filter( 'trx_addons_filter_get_list_input_hover', 'kryptex_filter_get_list_input_hover');
	function kryptex_filter_get_list_input_hover($list) {
		unset($list['accent']);
		unset($list['path']);
		unset($list['path']);
		unset($list['jump']);
...
if ( !function_exists( 'kryptex_filter_get_list_menu_hover' ) ) {
	add_filter( 'trx_addons_filter_get_list_menu_hover', 'kryptex_filter_get_list_menu_hover');
	function kryptex_filter_get_list_menu_hover($list) {
		unset($list['fade_box']);

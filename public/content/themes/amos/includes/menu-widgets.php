...
if (!class_exists('muizzWIM')) {
	...
	class muizzWIM {
		...
		public function __construct() {
			 ...
			 * @since 0.1.0
			 * 
			 * @param string $default_prefix The default prefix: 'muizz_wim'
			 */
			$this->attr_prefix = apply_filters('muizz_wim_attribute_prefix', $default_prefix);
		}
		...
		public function sidebar() {
			register_sidebar(array(
				'name'			=> __('Widgets in Menu', 'amos'),
				'id'			=> $this->prefix,
				"before_widget"	=> '<div id="%1$s" class="' . $this->attr_prefix . '_widget %2$s">',
				"after_widget"	=> '</div>',
				'description'	=> __('Widgets in this area will be shown on the edit menu screen.', 'amos'),
		...
		public function start_el($item_output, $item, $depth, $args) {
			...
			$sidebar = array_merge(
					...
					// our sidebar is at the index 'muizz_wim'
			 ...
			 * }
			 */
			$params = apply_filters('muizz_wim_widget_params', $params);
			
			$muizz_wim_widget = $wp_registered_widgets[$id];
			 ...
			 * }
			 */
			do_action('muizz_wim_pre_callback', $muizz_wim_widget);
		...
		public function menu_setup() {
			...
			if(function_exists('menu_for_theme'))
				menu_for_theme('meta_box');
		}
		...
		public function meta_box() {
			...
			if (empty($wp_registered_sidebars[$this->prefix]) 
				...
				// the default output
				$no_widgets_output = '<p>';
				$no_widgets_output .= sprintf(__('Please add a widget to the Widgets in Menu area', 'amos'));
				$no_widgets_output .= '</p>';
				 ...
				 */
				$no_widgets_output = apply_filters('muizz_wim_no_widgets_message', $no_widgets_output);
				...
				$output .= '<p style="display:none;" class="msg-muizz_sim">';
				// no text-domain, so that the Strings translated by WordPress are used
				$output .= __('Settings', 'amos')
						. ': '
			...
			// submit button
			?>
			<div class="muizz_wimdiv" id="muizz_wimdiv">
			<?php echo esc_attr($output); ?>
				<p class="button-controls">
					<span class="add-to-menu">
						<input type="submit"<?php wp_nav_menu_disabled_check($nav_menu_selected_id); ?> class="button-secondary submit-add-to-menu right" value="<?php esc_attr_e('Add to Menu', 'amos'); ?>" name="add-ya_wim-menu-item" id="submit-ya_wim" />
		...
		public function enqueue($hook) {
			...
			wp_enqueue_script(
					$this->prefix,
					get_template_directory_uri() . "/includes/admin-js/menu-widgets{$min}.js",
					array('nav-menu'),
					$this->version
					);
		}
		...
		function label($item) {
			if ($item->object === $this->prefix) {
				...
				// setup our label
				$item->type_label = __('Widget', 'amos');
			}
			return $item;
		}
...
$muizz_wim = new muizzWIM();
$muizz_wim_init = $muizz_wim->init();
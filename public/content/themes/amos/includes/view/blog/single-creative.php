...
global $for_element, $amos_redata;
...
if (have_posts()) :
    ...
    while (have_posts()) : the_post();
        ...
        ?> 
        <article id="post-<?php echo the_ID(); ?>" <?php echo post_class('blog-article creative-single '. $font_color.' intro-effect-push'); ?>>
            <div class="header_creative_single">
                <div class="overlay <?php echo esc_attr($amos_redata['page_header_menu_color']); ?>"></div>
                <?php if($post_format == 'gallery'){
                            $slider = new amos_slideshow($post_id, 'flexslider');

                            if($slider && $slider->slide_number > 0){
                                
                                $slider->img_size = 'amos_col1';
            ...
            <div class="title">
                <?php if($amos_redata['blog_info_date']): ?>
                        <div class="date"><?php _e('On', 'amos') ?> <?php echo get_the_date() ?></div>                           
                        <?php endif; ?>
                    <div class="info">
                        <?php if($amos_redata['blog_info_author']): ?>
                      <span class="author"><?php _e('By', 'amos') ?> <strong><?php echo get_the_author() ?></strong> </span> 
                        <?php endif; ?>
                        

                        <?php if($amos_redata['blog_info_comments']): ?>                   
                               | <span class="comments"><?php echo esc_attr($count) ?> <?php _e('Comments', 'amos') ?> </i></span>
                            <?php endif; ?>

                        
                    </div>
            <span class="categories"><?php echo wp_kses($cats , '', ''); ?></span>
            </div>
            </div>
            <div class="content">
                ...
                <div class="text">
                    ...
                    <?php if($post_format == 'quote' && !is_single()){ ?>

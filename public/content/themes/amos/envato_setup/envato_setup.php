	...
	class Envato_Theme_Setup_Wizard {
		...
		public function admin_menus() {
			...
			if( $this->is_submenu_page() ){
				...
				$add_subpage_function( $this->parent_slug, __( 'Setup Wizard','amos' ), __( 'Setup Wizard','amos' ), 'manage_options', $this->page_slug, array( $this, 'setup_wizard' ) );
			}else{
				add_theme_page( __( 'Setup Wizard','amos' ), __( 'Setup Wizard','amos' ), 'manage_options', $this->page_slug, array( $this, 'setup_wizard' ) );
			}
	...
	public function setup_wizard_header() {
		...
		<?php
				...
				if ( $image_url ) {
					...
					<img src="<?php echo esc_url($this->plugin_url); ?>images/logo.png" alt="<?php esc_attr_e('Envato install wizard', 'amos') ?>" /><?php
				} ?></a>
		</h1>
		<?php
		}
		...
		<?php if ( 'next_steps' === $this->step ) : ?>
			<a class="wc-return-to-dashboard" href="<?php echo esc_url( admin_url() ); ?>"><?php _e( 'Return to the WordPress Dashboard', 'amos' ); ?></a>
		<?php endif; ?>
		</body>
		<?php
...
//				exit;
				...
				?>
				<h1><?php printf( __( 'Welcome to the setup wizard for %s.', 'amos' ), wp_get_theme()); ?></h1>
				<p><?php _e('It looks like you may have recently upgraded to this theme. Great! This setup wizard will help ensure all the default settings are correct. It will also show some information about your new website and support options.', 'amos');?></p>
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button-primary button button-large button-next"><?php _e( 'Let\'s Go!', 'amos' ); ?></a>
					<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(),'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
					   class="button button-large"><?php _e( 'Not right now', 'amos' ); ?></a>
				</p>
				<?php
			} else if( get_option('envato_setup_complete', false) ){
				?>
				<h1><?php printf( __( 'Welcome to the setup wizard for %s.', 'amos' ), wp_get_theme()); ?></h1>
				<p><?php _e('It looks like you have already run the setup wizard. Below are some options: ', 'amos');?></p>
				<ul>
					<li>
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
						   class="button-primary button button-next button-large"><?php _e( 'Run Setup Wizard Again', 'amos' ); ?></a>
					</li>
					<li>
						<form method="post">
							<input type="hidden" name="reset-font-defaults" value="yes">
							<input type="submit" class="button-primary button button-large button-next" value="<?php esc_attr_e( 'Reset font style and colors', 'amos' ); ?>" name="save_step" />
				...
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(),'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
					   class="button button-large"><?php _e( 'Cancel', 'amos' ); ?></a>
				</p>
				<?php
			} else {
				?>
				<h1><?php printf( __( 'Welcome to the setup wizard for %s.', 'amos' ), wp_get_theme()); ?></h1>
				<p><?php printf( __( 'Thank you for choosing the %s theme from ThemeForest. This quick setup wizard will help you configure your new website. This wizard will install the required WordPress plugins, default content, logo and tell you a little about Help &amp; Support options. <br/>It should only take 5 minutes.' , 'amos' ), wp_get_theme()); ?></p>
				<p><?php _e( 'No time right now? If you don\'t want to go through the wizard, you can skip and return to the WordPress dashboard. Come back anytime if you change your mind!', 'amos' ); ?></p>

				<h2><?php printf( __( 'Warning ! Read Me', 'amos' ), wp_get_theme()); ?></h2>
				...
				<ul>
					...
					<li>2. Go to WP Dashboard > Amos > Import/Export   </li>
		...
		public function envato_setup_introduction_save(){
			...
			if(!empty($_POST['reset-font-defaults']) && $_POST['reset-font-defaults'] == 'yes'){
				...
				update_option('amos_redata', $data);

				$data1 = ReduxFrameworkInstances::get_instance('amos_redata')->options_defaults;

				update_option('amos_redata', $data1);
		...
		public function envato_setup_default_plugins() {
			...
			?>
			<h1><?php _e( 'Default Plugins', 'amos' ); ?></h1>
			<form method="post">
				...
				if ( count( $plugins['all'] ) ) {
					?>
					<p><?php _e( 'Your website needs a few essential plugins. The following plugins will be installed or updated:', 'amos' ); ?></p>
					...
					echo '<p><strong>'._e( 'Good news! All plugins are already installed and up to date. Please continue.','amos' ).'</strong></p>';
				} ?>

				<p><?php _e( 'You can add and remove plugins later on from within WordPress.', 'amos' ); ?></p>

				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button-primary button button-large button-next" data-callback="install_plugins"><?php _e( 'Continue', 'amos' ); ?></a>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next"><?php _e( 'Skip this step', 'amos' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
				</p>
			</form>
			<?php
		...
		public function ajax_plugins() {
			if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['slug'] ) ) {
				wp_send_json_error( array( 'error' => 1, 'message' => __( 'No Slug Found','amos' ) ) );
			}
			$json = array();
			...
			foreach ( $plugins['activate'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2' => -1,
						'message' => __( 'Activating Plugin','amos' ),
					);
					break;
				}
			}
			foreach ( $plugins['update'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2' => -1,
						'message' => __( 'Updating Plugin','amos' ),
					);
					break;
				}
			}
			foreach ( $plugins['install'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2' => -1,
						'message' => __( 'Installing Plugin','amos' ),
					);
					break;
				}
			}
			...
			if ( $json ) {
				...
				wp_send_json( array( 'done' => 1, 'message' => __( 'Success','amos' ) ) );
			}
			exit;
		...
		private function _content_default_get() {
			...
			foreach($available_content as $post_type => $post_data){
				if(count($post_data)){
					...
					$content[$post_type] = array(
						...
						'description' => sprintf( __( 'This will create default %s as seen in the demo.', 'amos' ), $post_type_title ),
						'pending' => __( 'Pending.', 'amos' ),
						'installing' => __( 'Installing.', 'amos' ),
						'success' => __( 'Success.', 'amos' ),
			...
			$content['widgets'] = array(
				'title' => __( 'Widgets', 'amos' ),
				'description' => __( 'Insert default sidebar widgets as seen in the demo.', 'amos' ),
				'pending' => __( 'Pending.', 'amos' ),
				'installing' => __( 'Installing Default Widgets.', 'amos' ),
				'success' => __( 'Success.', 'amos' ),
			...
			$content['options'] = array(
				'title' => __( 'Theme Option', 'amos' ),
				'description' => __( 'Configure default settings.', 'amos' ),
				'pending' => __( 'Pending.', 'amos' ),
				'installing' => __( 'Installing Default Settings.', 'amos' ),
				'success' => __( 'Success.', 'amos' ),
			...
			$content['attachments'] = array(
				'title' => __( 'Install Images', 'amos' ),
				'description' => __( 'Install Img and attachments', 'amos' ),
				'pending' => __( 'Pending.', 'amos' ),
				'installing' => __( 'Installing attachments.', 'amos' ),
				'success' => __( 'Success.', 'amos' ),
		    ...
		    $content['demo'] = array(
				'title' => __( 'Install Content', 'amos' ),
				'description' => __( 'Install Content of the Demo.', 'amos' ),
				'pending' => __( 'Pending.', 'amos' ),
				'installing' => __( 'Installing Demo.', 'amos' ),
				'success' => __( 'Success.', 'amos' ),
			...
			$content['menu'] = array(
				'title' => __( 'Menu', 'amos' ),
				'description' => __( 'Insert default menu as seen in the demo.', 'amos' ),
				'pending' => __( 'Pending.', 'amos' ),
				'installing' => __( 'Installing Default Menu.', 'amos' ),
				'success' => __( 'Success.', 'amos' ),
				'install_callback' => array( $this,'_content_install_menu' ),
			);

			$content['revslider'] = array(
				'title' => __( 'Revolution Slider', 'amos' ),
				'description' => __( 'Install Revolution Sliders.', 'amos' ),
				'pending' => __( 'Pending.', 'amos' ),
				'installing' => __( 'Installing Revolution Sliders... ', 'amos' ),
				'success' => __( 'Success.', 'amos' ),
		...
		public function envato_setup_default_content() {
			?>
			<h1><?php _e( 'Choose one demo to be installed', 'amos' ); ?></h1>
			<form method="post">
				<?php if($this->is_possible_upgrade()){ ?>
					<p><?php _e('It looks like you already have content installed on this website. If you would like to install the default demo content as well you can select it below. Otherwise just choose the upgrade option to ensure everything is up to date.', 'amos'); ?></p>
				<?php }else{ ?>
					<p><?php printf( __( 'It\'s time to insert some default content for your new WordPress website. Choose what you would like inserted below and click Continue. It is recommended to leave everything selected. Once inserted, this content can be managed from the WordPress admin dashboard. ', 'amos' ), '<a href="' . esc_url( admin_url( 'edit.php?post_type=page' ) ) . '" target="_blank">', '</a>' ); ?></p>
				<?php } ?>
				...
				<table class="envato-setup-pages" cellspacing="0">
					...
					<tr>
						<td class="check"> </td>
						<th class="item"><?php _e( 'Item', 'amos' ); ?></th>
						<th class="description"><?php _e( 'Description', 'amos' ); ?></th>
						<th class="status"><?php _e( 'Status', 'amos' ); ?></th>
					</tr>
					</thead>
				...
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button-primary button button-large button-next" data-callback="install_content"><?php _e( 'Continue', 'amos' ); ?></a>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next"><?php _e( 'Skip this step', 'amos' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
				</p>
			</form>
		...
		public function ajax_content() {
			...
			if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
				wp_send_json_error( array( 'error' => 1, 'message' => __( 'No content Found','amos' ) ) );
			}
			...
			if ( $json ) {
				...
				wp_send_json( array( 'error' => 1, 'message' => __( 'Error','amos' ) ) );
			}
...
//									echo "Post meta $meta_key was $meta_val \n\n";
		...
		private function _fetch_remote_file( $url, $post ) {
			...
			if ( !$upload || $upload['error'] ) {
				...
				if ( is_array( $response ) && !empty($response['body']) && $response['response']['code'] == '200' ) {
					...
					@unlink( $upload['file'] );
					return new WP_Error( 'import_file_error', __( 'Remote server did not respond', 'amos' ) );
				}
				...
				if ( isset( $headers['content-length'] ) && $filesize != $headers['content-length'] ) {
					...
					return new WP_Error( 'import_file_error', __( 'Remote file is incorrect size', 'amos' ) );
				}

				if ( 0 == $filesize ) {
					@unlink( $upload['file'] );

					return new WP_Error( 'import_file_error', __( 'Zero size file downloaded', 'amos' ) );
				}

				if ( ! empty( $max_size ) && $filesize > $max_size ) {
					@unlink( $upload['file'] );

					return new WP_Error( 'import_file_error', sprintf( __( 'Remote file is too large, limit is %s', 'amos' ), size_format( $max_size ) ) );
				}
			}
		...
		private function _content_install_widgets() {
			...
			if($options){
				...
				$this->amos_import_sidebars_widgets($file);
				$msg = '<br />'.__("Widgets Imported !", "amos");
				return true;
			}else{
				$msg = '<br />'.__("Your backup or demo doesn't contain sidebar_widgets.txt file.", "amos");
				return false;
			}
		...
		function amos_import_sidebars_widgets($file){
	    ...
	    public function modifyXml (){
		    ...
		    $path = get_template_directory() . '/includes/dummy_data/demos/'.$dir.'/' . $file.'.xml';
				...
				foreach($items as $item){
					...
					$wp_children->attachment_url = str_replace('http://amos.ellethemes.com/default/wp-content/uploads/', get_template_directory_uri().'/downloads/uploads/', $wp_children->attachment_url);
		...
		public function _content_install_menu(){
			...
			if($menus_data){
				...
				$this->message .= '<br />'.__("Menu Imported successfully", "amos");
				return true;
			}else{
				$this->message .= '<br />'.__("Your backup or demo doesn't contain sidebar_widgets.txt file.", "amos");
				return true;
			}
		...
		public function _content_install_revsliders(){
		   ...
		   $dir = $_POST['demo'];	
			...
			$slider_array = array('slider' => 'slider_zips/'.$dir.'/slider.zip');

			
			foreach ($slider_array as $sliders => $slider_url  ) {
				

					$uri = 'http://amos.ellethemes.com/'.$slider_url;
		...
		public function _content_install_options(){
			...
			if($options){
				update_option('amos_redata', $options);
				
				$msg = '<br />'.__("Options imported successfully", "amos");
				return true;
			}else{
				$msg = '<br />'.__("Your backup doesn't contain options.txt file.", "amos");
				return false;
			}
		}
	    ...
	    public function _content_install_demo() {
			...
			$path = get_template_directory() . '/includes/dummy_data/demos/'.$dir.'/'. $file.'.xml';
				...
				if(is_wp_error($value)){
					$this->message .= __("Error during Import. Try Again", "amos");
					
					return true;
				}
				else {
					$this->message .= __("Content has been imported successfully ", "amos");
		...
		public function read_file($file, $dir){
			...
			if ( file_exists($file) ) {
				...
				$this->message = __("File doesn't exist", "amos");
			}
		...
		public function envato_setup_logo_design() {
			 global $amos_redata;
			?>
			<h1><?php _e( 'Logo &amp; Design', 'amos' ); ?></h1>
			<form method="post">
				<p><?php printf( __( 'Please add your logo below. For best results, the logo should be a transparent PNG ( 466 by 277 pixels). The logo can be changed at any time from the Appearance > Customize area in your dashboard.', 'amos'), ''); ?></p>

				<table>
					<tr>
						<td>
							<div id="current-logo">
								<?php $image_url = $amos_redata['logo']['url'];
						...
						<td>
							<a href="#" class="button button-upload"><?php _e( 'Upload New Logo', 'amos' ); ?></a>
						</td>
					</tr>
				</table>
				...
				if( !$this->get_default_theme_style() || !$demo_styles){
					...
					<p><?php _e( 'Please choose the color scheme for this website. The color scheme (along with font colors &amp; styles) can be changed at any time from the Appearance > Customize area in your dashboard.' ,'amos' ); ?></p>
				...
				<p class="envato-setup-actions step">
					<input type="submit" class="button-primary button button-large button-next" value="<?php esc_attr_e( 'Continue', 'amos' ); ?>" name="save_step" />
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next"><?php _e( 'Skip this step', 'amos' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
				</p>
			</form>
		...
		public function envato_setup_logo_design_save() {
			...
			if ( $new_logo_id ) {
				...
				if ( $attr && ! empty( $attr[1] ) && ! empty( $attr[2] ) ) {
					$amos_redata['logo']['url'] = $attr[0];
					update_option('amos_redata', $amos_redata);
		...
		public function envato_setup_updates() {
			?>
			<h1><?php _e( 'Theme Updates', 'amos' ); ?></h1>
			<?php if ( function_exists( 'envato_market' ) ) { ?>
				<form method="post">
					...
					if ( count( $my_items ) ) {
						...
						<p class="envato-setup-actions step">
							<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next button-primary"><?php _e( 'Continue', 'amos' ); ?></a>
						</p>
						<?php
					} else {
						?>
						<p><?php _e( 'Please login using your ThemeForest account to enable Theme Updates. We update themes when a new feature is added or a bug is fixed. It is highly recommended to enable Theme Updates.', 'amos' ); ?></p>
						<p>When an update becomes available it will show in the Dashboard with an option to install.</p>
						<p>
						...
						<p class="envato-setup-actions step">
							<input type="submit" class="button-primary button button-large button-next" value="<?php esc_attr_e( 'Login with Envato', 'amos' ); ?>" name="save_step" />
							<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next"><?php _e( 'Skip this step', 'amos' ); ?></a>
							<?php wp_nonce_field( 'envato-setup' ); ?>
						</p>
					<?php } ?>
		...
		public function envato_setup_customize() {
			...
			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-primary button-large button-next"><?php _e( 'Continue', 'amos' ); ?></a>
			</p>
		...
		public function envato_setup_help_support() {
			...
			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-primary button-large button-next"><?php _e( 'Agree and Continue', 'amos' ); ?></a>
				<?php wp_nonce_field( 'envato-setup' ); ?>
			</p>
			<?php
		}
		...
		public function envato_setup_ready() {
			...
			<h1><?php _e( 'Your Website is Ready!', 'amos' ); ?></h1>
			...
			<div class="envato-setup-next-steps">
				<div class="envato-setup-next-steps-first">
					<h2><?php _e( 'Next Steps', 'amos' ); ?></h2>
					<ul>
						<li class="setup-product"><a class="button button-primary button-large" href="https://twitter.com/ellethemes" target="_blank"><?php _e( 'Follow @ellethemes on Twitter', 'amos' ); ?></a></li>
						<li class="setup-product"><a class="button button-next button-large" href="<?php echo esc_url( home_url('/') ); ?>"><?php _e( 'View your new website!', 'amos' ); ?></a></li>
					</ul>
				</div>
				<div class="envato-setup-next-steps-last">
					<h2><?php _e( 'More Resources', 'amos' ); ?></h2>
					<ul>
						<li class="documentation"><a href="<?php echo esc_url("http://ellethemes.com/documentation")?>" target="_blank"><?php _e( 'Read the Theme Documentation', 'amos' ); ?></a></li>
						<li class="howto"><a href="https://wordpress.org/support/" target="_blank"><?php _e( 'Learn how to use WordPress', 'amos' ); ?></a></li>
						<li class="rating"><a href="<?php echo esc_url("http://themeforest.net/downloads") ?>" target="_blank"><?php _e( 'Leave an Item Rating', 'amos' ); ?></a></li>
						<li class="support"><a href="<?php echo esc_url("http://ellethemes.com")?>" target="_blank"><?php _e( 'Get Help and Support', 'amos' ); ?></a></li>
					</ul>
				</div>
			</div>
			<?php
		...
		public function envato_market_admin_init() {
			...
			if ( ! empty( $_POST['oauth_session'] ) && ! empty( $_POST['bounce_nonce'] ) && wp_verify_nonce( $_POST['bounce_nonce'], 'envato_oauth_bounce_' . $this->envato_username ) ) {
				...
				if ( is_wp_error( $response ) ) {
					...
					$class = 'error';
					echo "<div class=\"$class\"><p>".sprintf( __( 'Something went wrong while trying to retrieve oauth token: %s','amos' ), $error_message ).'</p></div>';
				} else {
			...
			add_settings_section(
				...
				sprintf( __( 'Login for %s updates', 'amos' ), $this->envato_username ),

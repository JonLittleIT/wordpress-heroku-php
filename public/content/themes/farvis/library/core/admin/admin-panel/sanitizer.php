<?php
	...
	function farvis_sanitize_text( $input ) {
		return wp_kses_post( esc_html( $input ) );
	}

	function farvis_sanitize_checkbox( $input ) {
		if ( $input == 'on' ) {
			return 'on';
		} else {
			return '';
	...
	function farvis_sanitize_per_page( $input ) {
		if ( is_numeric( $input ) && $input != '0' ) {
			return absint( $input );
		} else return '';
	}

	function farvis_sanitize_absinteger( $input ) {
		if ( is_numeric( $input ) && $input != '0' ) {
			return absint( $input );
		} else return '';
	}

	function farvis_sanitize_email( $email ) {
		if (is_email( $email ) ) {
			return $email;
		} else {
			return '';
	...
	function farvis_sanitize_layout( $value ) {
	...
	function farvis_sanitize_loader( $value ) {
	...
	function farvis_sanitize_onoff( $value ) {
	...
	function farvis_sanitize_sidebar_blog_type( $value ) {
	...
	function farvis_sanitize_sidebar_blog_content( $value ) {
	...
	function farvis_sanitize_header_type( $value ) {
	...
	function farvis_sanitize_overlay_opacity( $value ) {
	...
	function farvis_sanitize_portfolio_type( $value ) {
	...
	function farvis_sanitize_portfolio_perrow( $value ) {
	...
	function farvis_sanitize_sidebar_portfolio_type( $value ) {
		if ( ! in_array( $value, array( '1', '2', '3' ) ) ) {
			$value = '2';
		}
	...
	function farvis_sanitize_sidebar_portfolio_content( $value ) {
	...
	function farvis_sanitize_animation( $value ) {
	...
	function farvis_sanitize_footer_block($input) {
		$valid = farvis_get_staticblock_option_array();

...
class Setup
{
    ...
    public function old_core_check() {
        ...
        if(0 < sizeof($ActivatedOldCore)){
            wp_die(
                esc_html__( "The old version of Core plugin has been deactivated due to compatibility. Please activate Javo Core plugin to the newest version.", 'listopia' ) .
                sprintf( '<p><a href="%1$s" target="_self" title="%2$s">%2$s</a></p>', admin_url( 'plugins.php' ), esc_html__( "Go to plugin page", 'listopia' ) )
            );
        }
    }

...
/**
 * Beehive_Breadcrumb_Trail class.
 *
 * @since 1.0.0
 */
class Beehive_Breadcrumb_Trail {
	...
	public function trail() {
		...
		if ( 0 < $item_count ) {
			...
			foreach ( $this->items as $item ) {
				...
				$item = ! empty( $matches ) ? sprintf( '%s<span itemprop="title">%s</span>%s', $matches[1], $matches[2], $matches[3] ) : sprintf( '%s', $item );

				// Wrap the item with its itemprop.
				$item = ! empty( $matches )
					? preg_replace( '/(<a.*?)([\'"])>/i', '$1$2 itemprop=$2url$2>', $item )
					: sprintf( '<span>%s</span>', ( is_single() && 'post' === get_post_type() && 'full' === beehive()->layout->get() ) ? esc_html__( 'Single', 'beehive' ) : $item );
				...
				$attributes = ! empty( $matches ) ? 'itemscope itemtype="http://data-vocabulary.org/Breadcrumb" class="' . $item_class . '"' : 'class="' . $item_class . '"';
			...
			$breadcrumb = sprintf(
				'<%1$s role="navigation" aria-label="%2$s" class="beehive-breadcrumb breadcrumbs">%3$s%4$s%5$s</%1$s>',
	...
	protected function set_labels() {
		...
		$defaults = array(
			'browse'              => esc_html__( 'Browse:', 'beehive' ),
			'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'beehive' ),
			'home'                => esc_html__( 'Home', 'beehive' ),
			'error_404'           => esc_html__( '404 Not Found', 'beehive' ),
			'archives'            => esc_html__( 'Archives', 'beehive' ),
			// Translators: %s is the search query.
			'search'              => esc_html__( 'Search results for: %s', 'beehive' ),
			// Translators: %s is the page number.
			'paged'               => esc_html__( 'Page %s', 'beehive' ),
			// Translators: %s is the page number.
			'paged_comments'      => esc_html__( 'Comment Page %s', 'beehive' ),
			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute'      => esc_html__( 'Minute %s', 'beehive' ),
			// Translators: Weekly archive title. %s is the week date format.
			'archive_week'        => esc_html__( 'Week %s', 'beehive' ),
		...
		if ( false === $done_post_type && 1 === count( $taxonomy->object_type ) && post_type_exists( $taxonomy->object_type[0] ) ) {
			...
			if ( 'post' === $taxonomy->object_type[0] ) {
				...
				if ( 'posts' !== get_option( 'show_on_front' ) && 0 < $post_id ) {
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_permalink( $post_id ) ), wp_kses_post( get_the_title( $post_id ) ) );
				}
	...
	protected function add_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'beehive' ) ) );
		}
	}
	...
	protected function add_minute_archive_items() {
		...
		// Add the minute item.
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'beehive' ) ) );
		}
	}
	...
	protected function add_hour_archive_items() {
		...
		// Add the hour item.
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'beehive' ) ) );
		}
	}
	...
	protected function add_day_archive_items() {
		...
		$year  = sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'beehive' ) ) );
		$month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'beehive' ) ) );
		$day   = sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'beehive' ) ) );
	...
	protected function add_week_archive_items() {
		...
		$year = sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'beehive' ) ) );
		$week = sprintf( $this->labels['archive_week'], get_the_time( esc_html_x( 'W', 'weekly archives date format', 'beehive' ) ) );
	...
	protected function add_post_parents( $post_id ) {
		...
		while ( $post_id ) {
			...
			$parents[] = sprintf( '<a href="%s">%s</a>', esc_url( get_permalink( $post_id ) ), wp_kses_post( get_the_title( $post_id ) ) );
	...
	protected function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag ) {
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'beehive' ) ) ) );
				} elseif ( '%monthnum%' == $tag ) { // If using the %monthnum% tag, add a link to the monthly archive.
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'beehive' ) ) ) );
				} elseif ( '%day%' == $tag ) { // If using the %day% tag, add a link to the daily archive.
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'beehive' ) ) ) );

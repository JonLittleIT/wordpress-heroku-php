...
add_action('widgets_init', 'bitther_information');

function bitther_information()
{
...
class information_widget extends WP_Widget
{
    ...
    {
        /* Widget settings. */
        $widget_ops = array('classname' => 'information_widget', 'description' => esc_html__('Easy add image for sidebar.', 'bitther'));

        /* Widget control settings. */
        $control_ops = array('width' => 250, 'height' => 350, 'id_base' => 'information_widget');

        /* Create the widget. */
        parent::__construct('information_widget', esc_html__('+NA: Image', 'bitther'), $widget_ops, $control_ops);
    }
    ...
    function widget($args, $instance)
    {
        ...
        ?>
        <div class="bitther-image-content">
            <?php if ($image) {
                ?>
                <a class="clearfix" href="<?php echo esc_url($link); ?>">
                    <img class="bitther-image" src="<?php echo esc_url($image)?>" alt="<?php echo esc_attr('bitther-image'); ?>" />
                </a>
            <?php } ?>
        </div>
        <?php
    ...
    function form($instance)
    {
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('link')); ?>"><?php echo esc_html_e('Url:', 'bitther'); ?></label>
